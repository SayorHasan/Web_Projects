        ...
        <div class="container">
            ...
            <div class="edit-header">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        <h1 class="edit-title">
                            <i class="fas fa-edit me-3"></i>Edit Turf
                        </h1>
                        <p class="edit-subtitle">Update information for {{ $turf->name }}</p>
                    </div>
                    <a href="{{ route('admin.turfs.show', $turf) }}" class="back-btn">
            ...
            <!-- Edit Form -->
            <div class="edit-form">
                <form action="{{ route('admin.turfs.update', $turf) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" id="name" name="name" class="form-control @error('name') is-invalid @enderror" 
                                   value="{{ old('name', $turf->name) }}" placeholder="Enter turf name" required>
                            @error('name')
                        ...
                        <div class="form-group">
                            ...
                            @php $sports = ['football','cricket','tennis','basketball','volleyball','badminton'];
                                 $selected = old('sport_types', $turf->sport_list);
                            @endphp
                            <div class="features-section">
                                @foreach($sports as $sport)
                                <div class="feature-item">
                                    <input type="checkbox" id="sport_{{ $sport }}" name="sport_types[]" value="{{ $sport }}" class="feature-checkbox"
                                           {{ in_array($sport, $selected) ? 'checked' : '' }}>
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="description" name="description" rows="4" class="form-control @error('description') is-invalid @enderror" 
                                  placeholder="Describe the turf and its features" required>{{ old('description', $turf->description) }}</textarea>
                        @error('description')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="number" id="price_per_hour" name="price_per_hour" step="0.01" min="0" 
                                   ...
                                   value="{{ old('price_per_hour', $turf->price_per_hour) }}" placeholder="0.00" required>
                        ...
                        <div class="form-group">
                            ...
                            <select id="status" name="status" class="form-control @error('status') is-invalid @enderror" required>
                                ...
                                <option value="available" {{ old('status', $turf->status) == 'available' ? 'selected' : '' }}>Available</option>
                                <option value="maintenance" {{ old('status', $turf->status) == 'maintenance' ? 'selected' : '' }}>Maintenance</option>
                                <option value="booked" {{ old('status', $turf->status) == 'booked' ? 'selected' : '' }}>Booked</option>
                            </select>
                            @error('status')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                    ...
                    <div class="current-image-section">
                        ...
                        <img src="{{ asset('storage/' . $turf->image_path) }}" alt="{{ $turf->name }}" class="current-image">
                        <p class="current-image-label">Current turf image</p>
                    </div>
                        ...
                        <div class="features-section">
                            <div class="feature-item">
                                <input type="checkbox" id="lighting" name="features[]" value="lighting" class="feature-checkbox" 
                                       {{ in_array('lighting', old('features', $turf->features ?? [])) ? 'checked' : '' }}>
                            ...
                            <div class="feature-item">
                                <input type="checkbox" id="parking" name="features[]" value="parking" class="feature-checkbox"
                                       {{ in_array('parking', old('features', $turf->features ?? [])) ? 'checked' : '' }}>
                            ...
                            <div class="feature-item">
                                <input type="checkbox" id="equipment" name="features[]" value="equipment" class="feature-checkbox"
                                       {{ in_array('equipment', old('features', $turf->features ?? [])) ? 'checked' : '' }}>
                            ...
                            <div class="feature-item">
                                <input type="checkbox" id="changing_rooms" name="features[]" value="changing_rooms" class="feature-checkbox"
                                       {{ in_array('changing_rooms', old('features', $turf->features ?? [])) ? 'checked' : '' }}>
                            ...
                            <div class="feature-item">
                                <input type="checkbox" id="refreshments" name="features[]" value="refreshments" class="feature-checkbox"
                                       {{ in_array('refreshments', old('features', $turf->features ?? [])) ? 'checked' : '' }}>
                            ...
                            <div class="feature-item">
                                <input type="checkbox" id="coaching" name="features[]" value="coaching" class="feature-checkbox"
                                       {{ in_array('coaching', old('features', $turf->features ?? [])) ? 'checked' : '' }}>
                    ...
                    <div class="form-actions">
                        ...
                        <button type="submit" class="submit-btn">
                            <i class="fas fa-save me-2"></i>Update Turf
                        </button>
                    </div>
                </form>
            </div>
